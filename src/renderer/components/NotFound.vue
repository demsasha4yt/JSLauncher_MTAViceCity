<template>
    <div class="init">
        ...
        <div class="init-wrap">
           <div class="NotFound">
               <h2>Установка:</h2>
               <p>Выберите директорию для установки</p>
           </div>
            ...
            <button @click="installGta" class="main-btn launch-btn">УСТАНОВИТЬ</button>
            <div class="havegta">
                <router-link to="havegta"><span id="suricen">*</span><span id="nadp"> у меня уже есть установленая MTA Vice City</span></router-link>
            </div>
        </div>
    </div>
</template>
    ...
    export default {
        ...
        data: function () {
            return {
                ...
                installPath: "C:/Games/MTA Vice City/"
            }
        },
        computed: {
        ...
        mounted () {
            ...
            this.installPath = this.$store.getters.GtaPath;
        },
        methods: {
            select: function () {
                const {dialog} = require('electron').remote;
                let dialogResponse = dialog.showOpenDialog({properties: ['openDirectory']});
                if (dialogResponse)
                    this.installPath = path.join(dialogResponse[0], 'MTA Vice City');
            },
            installGta: function () {
                let path = this.installPath;
                if (path === null)
                    return null;
                else {
                    this.$store.dispatch('startInstallGta', path);
                }
            }
        }
    }
